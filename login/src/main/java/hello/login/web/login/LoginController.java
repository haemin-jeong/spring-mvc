...
//    @PostMapping("/login")
    public String login(@Valid @ModelAttribute("loginForm") LoginForm form, BindingResult bindingResult, HttpServletResponse response) {
        ...
        //쿠키에 시간 정보를 설정하지 않으면 세션쿠키(브라우저 종료시까지만 유지되는 쿠키)
...
//    @PostMapping("/login")
    ...
    public String loginV3(@Valid @ModelAttribute LoginForm form, BindingResult bindingResult, HttpServletRequest request) {
        ...
        //request.getSession(true) : 세션이 있으면 기존 세션 반환, 없으면 새로운 세션 생성해서 반환, 파라미터 생략시 기본값
...
//    @PostMapping("/logout")
    ...
    private void expireCookie(HttpServletResponse response, String cookieName) {
        ...
        cookie.setMaxAge(0); //max-age를 0으로하여 쿠키 소멸
